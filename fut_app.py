...
def final_decision(long_score, short_score):
    ...
    motivos.append("✅ Score mínimo alcanzado" if score_ok else "❌ Score insuficiente")

    motivos.append("✅ Riesgo válido" if risk_ok else "❌ Riesgo mayor a 1%")

    motivos.append("✅ RR válido" if rr_ok else "❌ RR insuficiente")

    motivos.append("✅ Hay ruptura de estructura" if ruptura_ok else "❌ Sin ruptura de estructura")



    if decision == "LONG":

        motivos.append("✅ Dirección final: LONG")

    elif decision == "SHORT":

        motivos.append("✅ Dirección final: SHORT")

    else:

        motivos.append("⚪ Dirección final: NO OPERAR")
...
try:
    ...
    decision = final_decision(long_score, short_score)



    sl, tp, risk_points = technical_levels(decision, df_5m, price)
    ...
    motivos.append(f"24H = {t24}")

    motivos.append(f"4H = {t4}")

    motivos.append(f"1H = {t1}")

    motivos.append(f"15M = {t15}")

    motivos.append(f"5M = {t5}")
    ...
    if decision != "NO_OPERAR":
        ...
        st.info("No se calcula SL/TP porque el sistema no da permiso para operar.")
