...
def detectar_zonas_daytrade(df_15m, asia_high=None, london_high=None, max_zonas=5):
    ...
    def agregar_zona(nombre, nivel, ancho_atr, peso, motivo, tipo):
        ...
        desde = nivel - atr * ancho_atr

        hasta = nivel + atr * ancho_atr

        distancia_atr = abs(precio - nivel) / atr
        ...
        if distancia_atr <= 4:
            ...
            zonas.append({
                ...
                "score": max(0, peso + (4 - distancia_atr)),
    ...
    if "ema20" in df.columns:
        ...
        agregar_zona("EMA20", df["ema20"].iloc[-1], 0.15, 12, "EMA20 cercana", "dinamica")

    if "ema50" in df.columns:

        agregar_zona("EMA50", df["ema50"].iloc[-1], 0.18, 14, "EMA50 cercana", "dinamica")

    # Asia / Londres

    agregar_zona("Máximo Asia", asia_high, 0.20, 16, "Liquidez de Asia", "liquidez")

    agregar_zona("Máximo Londres", london_high, 0.20, 18, "Liquidez de Londres", "liquidez")
    ...
    for i in range(2, len(df)):
        ...
        if low_3 > high_1:
            ...
            tam_atr = abs(fvg_hasta - fvg_desde) / atr

            dist_atr = abs(precio - centro) / atr

            if tam_atr >= 0.20 and dist_atr <= 4:
                ...
                zonas.append({
                    ...
                    "score": 20 + min(tam_atr * 5, 10) + max(0, 4 - dist_atr),

                    "motivos": ["Bearish FVG / ineficiencia"],

                    "tipo": "fvg_bajista",

                    "distancia_atr": dist_atr

                })

    # Unificar zonas cercanas

    zonas = sorted(zonas, key=lambda x: x["score"], reverse=True)
    ...
    for z in zonas:
        ...
        for f in finales:
            ...
            se_solapan = not (z["hasta"] < f["desde"] or z["desde"] > f["hasta"])
    ...
    for z in finales:
        ...
        score_100 = min(100, int(z["score"] * 3))
        ...
        if "fvg_bajista" in z["tipo"] or "resistencia" in z["tipo"] or "liquidez" in z["tipo"]:

            sesgo = "SHORT potencial si hay rechazo"

            activadores = ["Sweep de liquidez", "Vela de rechazo", "CHoCH bajista", "Pérdida de EMA20"]

        else:

            sesgo = "Zona de decisión"

            activadores = ["Confirmar estructura", "Confirmar volumen", "Esperar ruptura o rechazo"]
        ...
        resultado.append({
            ...
            "estrellas": "⭐" * max(1, min(5, round(score_100 / 20))),
...
def render_executive_card(
    ...
    <div class="executive-card {status_class}">
        ...
        <div class="exec-subtitle">
            Asistente operativo BTC intradía · decisión, riesgo y niveles principales en una sola vista.
        </div>
        ...
        <div class="exec-grid">
            <div class="exec-box">
                <div class="exec-label">Entrada</div>
                <div class="exec-value">{entrada_txt}</div>
            </div>
            <div class="exec-box">
                <div class="exec-label">Stop Loss</div>
                <div class="exec-value">{sl_txt}</div>
            </div>
            <div class="exec-box">
                <div class="exec-label">Take Profit</div>
                <div class="exec-value">{tp_txt}</div>
            </div>
            <div class="exec-box">
                <div class="exec-label">RR real</div>
                <div class="exec-value">{rr_txt}</div>
            </div>
            <div class="exec-box">
                <div class="exec-label">Riesgo</div>
                <div class="exec-value">{riesgo_txt}</div>
            </div>
            <div class="exec-box">
                <div class="exec-label">Objetivo</div>
                <div class="exec-value">{profit_txt}</div>
            </div>
            <div class="exec-box">
                <div class="exec-label">Score LONG / SHORT</div>
                <div class="exec-value">{long_score}/10 · {short_score}/10</div>
            </div>
            <div class="exec-box">
                <div class="exec-label">FVG</div>
                <div class="exec-value">{fvg_label}</div>
            </div>
        </div>
        {motivos_html}
    </div>
...
try:
    ...
    **Zona {i} {z['estrellas']} — {z['score']}/100**  
