...
modo_operativo = st.sidebar.selectbox(
    "Modo de operación",

    ["Conservador", "Normal", "Agresivo", "Scalping agresivo"],

    index=1,

    key="modo_operativo_select"
)
...
def run_simple_backtest(timeframe, period_label, estrategia, min_score, rr_target, min_rr, max_atr_multiplier, use_time_filter, avoid_weekends_filter, fvg_min_atr_param, fvg_max_distance_atr_param, use_fvg_filter_param):
    ...
    for i in range(220, len(df) - horizon, step):
        ...
        valid_bt, motivos_bt = validate_trade(
            ...
            fvg_bt, use_fvg_filter_param, fvg_max_distance_atr_param, df_1h_bt
        )
        if not valid_bt:
            continue
...
def build_market_update_msg(color, decision, candidate_direction, symbol, estrategia, price,
                            long_score, short_score, rsi, atr, t4, t1, t15, t5,
                            structure, escenario, motivos, current_candle_time):
    if color == "GREEN":
...
try:
    ...
    market_update_msg = build_market_update_msg(
        market_color, decision, candidate_direction, SYMBOL, estrategia, price,
        long_score, short_score, rsi_15, atr_15m, t4, t1, t15, t5,
        structure, escenario, motivos_bloqueo, current_candle_time
    )

    auto_saved_now = False
    alert_events = []

    # Resumen automático por intervalo. Depende del auto-refresh o de que la página se vuelva a ejecutar.
    ...
    if telegram_enabled or discord_enabled:
        ...
        with st.expander("Textos de alerta actuales"):
            ...
            st.code(market_update_msg, language="text")
