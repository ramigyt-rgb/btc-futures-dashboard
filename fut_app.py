...
def is_allowed_trading_time(ts, use_filter=True, avoid_weekends=True):
    if pd.isna(ts):
        return False
    # Horarios UTC aproximados: Londres 07-10, NY 13-16. BTC opera 24/7, esto filtra ruido.
...
def outcome_from_future(future_df, direction, sl, tp):
    ...
    for _, row in future_df.iterrows():
        ...
        if hit_sl and hit_tp:
            return "SL", -1.0  # criterio conservador si toca ambos en la misma vela
        if hit_tp:
            return "TP", rr_target
        if hit_sl:
            return "SL", -1.0
...
def performance_stats(results):
    ...
    closed = results[results["resultado"].isin(["TP", "SL"])]
    if closed.empty:
        return {"operaciones": 0, "winrate": 0, "profit_factor": 0, "drawdown": 0, "expectancy": 0, "rr_promedio": 0}
...
def run_simple_backtest(timeframe, period_label, estrategia, min_score, rr_target, min_rr, max_atr_multiplier, use_time_filter, avoid_weekends_filter):
    ...
    rows = []
    step = 4 if timeframe == "15m" else 12  # evita sobrecontar señales pegadas
    horizon = 96 if timeframe == "15m" else 288  # 24 horas
    for i in range(220, len(df) - horizon, step):
        hist = df.iloc[:i + 1].copy()
        df_15_bt = hist if timeframe == "15m" else hist.resample("15min", on="time").agg({"open":"first","high":"max","low":"min","close":"last","volume":"sum"}).dropna().reset_index()
        df_15_bt = add_indicators(df_15_bt)
        df_5_bt = hist
        df_1h_bt = add_indicators(hist.resample("1h", on="time").agg({"open":"first","high":"max","low":"min","close":"last","volume":"sum"}).dropna().reset_index())
        df_4h_bt = add_indicators(hist.resample("4h", on="time").agg({"open":"first","high":"max","low":"min","close":"last","volume":"sum"}).dropna().reset_index())
        df_1d_bt = add_indicators(hist.resample("1d", on="time").agg({"open":"first","high":"max","low":"min","close":"last","volume":"sum"}).dropna().reset_index())
        if len(df_1h_bt) < 210 or len(df_4h_bt) < 60 or len(df_15_bt) < 210:
            continue
        ...
        result, r_mult = outcome_from_future(future, direction_bt, sl_bt, tp_bt)
        rows.append({
...
def validate_saved_signals(historial, current_df_5m, hours_after=24):
    ...
    for idx, row in h.iterrows():
        if str(row.get("resultado", "nan")) in ["TP", "SL", "NINGUNO"]:
            continue
        try:
        ...
        result, r_mult = outcome_from_future(future, direction, float(row["sl"]), float(row["tp"]))
...
try:
    ...
    tp_atr_multiple = tp_distance_calc / atr_15m if not pd.isna(tp_disntance_calc) and not pd.isna(atr_15m) and atr_15m > 0 else np.nan
    ...
    q4.metric(
        "Distancia al TP",
        f"{tp_distance_cacl:,.0f} pts" if not pd.isna(tp_distance_calc) else "-"
    ...
    r5.metric("RR real", f"{rr_real:.2f}" if not pd.isna(rr_real) else "—")
    st.divider()
    ...
    if os.path.exists(SIGNALS_FILE):
        ...
        historial.to_csv(SIGNALS_FILE, index=False)
        stats_hist = performance_stats(historial.rename(columns={"rr_real": "rr"})) if "resultado" in historial.columns else {}
