...
def check_login(usuario, password):
    ...
    # Compatibilidad por si tenías users.csv viejo con columna password sin hash.
...
with st.sidebar.expander("🔔 Alertas reales", expanded=False):
    st.caption("Opcional. Pegá tus webhooks/tokens. Si queda vacío, solo muestra texto para copiar.")
    telegram_enabled = st.checkbox("Activar Telegram", value=False)
    telegram_bot_token = st.text_input("Telegram bot token", value="", type="password")
...
def signal_key(row):
    # Una señal por vela/estrategia/dirección. Evita duplicar en cada rerun.
...
def send_telegram_message(token, chat_id, text):
    if not token or not chat_id or requests is None:
        return False, "Falta token/chat_id o librería requests."
    try:
...
def send_discord_message(webhook_url, text):
    ...
    try:
        r = requests.post(webhook_url, json={"content": text}, timeout=8)
        return r.ok, r.text[:200]
    except Exception as e:
        return False, str(e)
...
try:
    ...
    alert_txt = (
        ...
        f"TP: {tp_calc:,.0f}\n"
        f"RR: {rr_real:.2f}\n"
        f"Setup: {quality_label} {quality_score}/10"
    )
    ...
    if auto_capture_enabled and trade_valid:
        ...
        if row_auto["signal_key"] not in existing_keys:
            ...
            if telegram_enabled and telegram_bot_token and telegram_chat_id:
                send_telegram_message(telegram_bot_token, telegram_chat_id, alert_txt)
            if discord_enabled and discord_webhook_url:
                send_discord_message(discord_webhook_url, alert_txt)
    ...
    if telegram_enabled or discord_enabled:
        st.info("Alerta lista. Si configuraste token/webhook, se envía automáticamente cuando aparece una señal válida nueva.")
        with st.expander("Texto de alerta"):
            st.code(alert_txt)
    ...
    with st.expander("📘 Reglas del sistema"):
        ...
        - Telegram/Discord pueden enviar alertas reales si configurás token/webhook.
        """)
