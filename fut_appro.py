...
with st.sidebar.expander("🔔 Alertas estratégicas", expanded=False):
    ...
    st.divider()
    alert_market_updates = st.checkbox("Enviar resumen de mercado cada 5 min", value=True, key="alert_market_updates_chk")
...
def detect_fvg_zones(df, min_atr_mult=0.20, lookback=120):
    ...
    if out.empty:
        return out
    # Marcar mitigado: después de crearse, si el precio tocó el midpoint del FVG.
...
def fvg_state(price, atr, fvg_df, direction, max_distance_atr=1.5):
    ...
    if inside:
        label = f"DENTRO {z['tipo']}"
    elif valid:
        label = f"CERCA {z['tipo']}"
    else:
        label = f"LEJOS {z['tipo']}"
...
def position_size(entry, sl, account_usd, risk_pct, leverage):
    """
    Corrección profesional del NOTIONAL:
    1) Calcula tamaño por riesgo: risk_usd / distancia SL.
    2) Calcula el nocional requerido.
    3) Lo limita al máximo permitido por capital y apalancamiento: cuenta * leverage.
    4) Si lo limita, recalcula riesgo real. Así nunca muestra 8000 USDT si con 500 x5 el máximo es 2500.
    """
...
def strategy_filter(estrategia, direction, df_15m, df_5m, levels, structure, market_bias, fvg_df=None, use_fvg_filter=False, fvg_max_distance_atr=1.5):
    motivos = []
    price = df_5m["close"].iloc[-1]
    last = df_5m.iloc[-1]
    ema50 = df_15m["ema50"].iloc[-1]
    ...
    if estrategia == "Tendencia + Liquidez":
        ...
        if direction == "LONG" and not (trend_state(df_1h_global) == "BULL" or trend_state(df_15m) == "BULL"):
            motivos.append("FVG + Tendencia: falta tendencia alcista en 1H/15M.")
        if direction == "SHORT" and not (trend_state(df_1h_global) == "BEAR" or trend_state(df_15m) == "BEAR"):
...
def validate_trade(estrategia, direction, long_score, short_score, min_score, risk_pct, rr_real, min_rr,
                   ...
                   fvg_df=None, use_fvg_filter=False, fvg_max_distance_atr=1.5):
    motivos = []
    ...
    motivos.extend(strategy_filter(estrategia, direction, df_15m, df_5m, levels, structure, market_bias, fvg_df, use_fvg_filter, fvg_max_distance_atr))
...
def run_simple_backtest(timeframe, period_label, estrategia, min_score, rr_target, min_rr, max_atr_multiplier, use_time_filter, avoid_weekends_filter):
    ...
    for i in range(220, len(df) - horizon, step):
        ...
        fvg_bt = detect_fvg_zones(df_15_bt, fvg_min_atr, 120)
        price_bt = df_5_bt["close"].iloc[-1]
        structure_bt = structure_state(df_5_bt)
        long_score_bt = score_direction("LONG", df_4h_bt, df_1h_bt, df_15_bt, df_5_bt, levels_bt, fvg_bt, fvg_max_distance_atr)
        short_score_bt = score_direction("SHORT", df_4h_bt, df_1h_bt, df_15_bt, df_5_bt, levels_bt, fvg_bt, fvg_max_distance_atr)
        ...
        valid_bt, motivos_bt = validate_trade(
            ...
            fvg_bt, use_fvg_filter, fvg_max_distance_atr
        )
        if not valid_bt:
            continue
        ...
        rows.append({
            ...
            "fvg": fvg_zone_bt.get("tipo") if fvg_zone_bt else "SIN_FVG",
...
def build_market_update_msg(color, decision, candidate_direction, symbol, estrategia, price, sl, tp, rr, quality_label, quality_score,

                            long_score, short_score, rsi, atr, t4, t1, t15, t5, structure, escenario, fvg_label,

                            motivos, current_candle_time):
    ...
    Dirección calculada: <b>{candidate_direction}</b>



    Precio: <b>{fmt0(price)}</b>
    ...
    Calidad: <b>{quality_label} {quality_score}/10</b>
    ...
    RSI 15M: {format_number(rsi, 2)} — {rsi_state(rsi)}
    ...
    if color == "GREEN":
        ...
        accion = "Puede haber oportunidad válida según reglas."

    elif color == "RED":

        header = "🔴 <b>ALERTA ROJA — NO OPERAR / RIESGO ALTO</b>"

        accion = "Mercado peligroso o condiciones débiles. Esperar."

    else:

        header = "🟡 <b>ALERTA AMARILLA — SETUP EN FORMACIÓN</b>"

        accion = "Hay intención, pero todavía falta confirmación."



    motivos_txt = "\n".join([f"• {m}" for m in motivos[:5]]) if motivos else "• Sin bloqueos graves detectados."
...
st.caption("Sistema objetivo: tendencia + liquidez + estructura + score + riesgo + FVG + backtesting + alertas estratégicas. No es consejo financiero.")

try:
    df_1d, df_4h, df_1h, df_15m, df_5m = load_all_data()
    # Global solo para compatibilidad dentro de strategy_filter FVG.
    df_1h_global = df_1h
    ...
    sl_calc, tp_calc, risk_points_calc = technical_levels(candidate_direction, df_5m, price, rr_target, fvg_zone)
    ...
    trade_valid, motivos_bloqueo = validate_trade(
        ...
        fvg_df, use_fvg_filter, fvg_max_distance_atr
    )
    ...
    loss_pct = (risk_usd / account_usd) * 100 if account_usd else 0
    ...
    valid_msg = build_valid_signal_msg(decision, SYMBOL, estrategia, price, sl_calc, tp_calc, rr_real, quality_label, quality_score, long_score, short_score, rsi_15, atr_15m, current_candle_time, fvg_label)
    ...
    market_update_msg = build_market_update_msg(
    ...
    alert_events = []
    # Enviar resumen automático cada 5 minutos

    market_update_key = f"MARKET_UPDATE_{datetime.utcnow().strftime('%Y-%m-%d_%H_%M')[:15]}"
    ...
    if telegram_enabled:
        ...
        if minute_now % 5 == 0:
            ...
            if ok:
                ...
                alert_events.append("📡 Resumen de mercado 5M enviado a Telegram.")
    ...
    if show_fvg_zones and not fvg_df.empty:
        open_fvg = fvg_df[~fvg_df["mitigated"].astype(bool)].tail(8)
    ...
    fig.update_layout(template="plotly_white", height=500 if mobile_compact else 700, xaxis_rangeslider_visible=False, title="BTCUSDT 15M - Velas + EMAs + Liquidez + FVG + SL/TP", margin=dict(l=20, r=20, t=50, b=20))
