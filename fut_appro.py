...
def send_alert_once(alert_key, tipo, mensaje, telegram_on, discord_on, discord_url="", token_override="", chat_id_override=""):
    ...
    if discord_on and discord_url:
        ok, resp = send_discord_message(discord_url, mensaje.replace("<b>", "**").replace("</b>", "**"))
...
validate_after_hours = st.sidebar.number_input("Validar señales después de horas", min_value=1, max_value=72, value=24, step=1)
...
auto_refresh_enabled = st.sidebar.checkbox("Auto actualizar para alertas", value=True)
auto_refresh_seconds = st.sidebar.number_input("Actualizar cada segundos", min_value=30, max_value=900, value=60, step=30)
...
with st.sidebar.expander("🔔 Alertas estratégicas", expanded=False):
    ...
    telegram_enabled = st.checkbox("Activar Telegram", value=True)
    telegram_bot_token = st.text_input("Telegram bot token opcional", value="", type="password")
    telegram_chat_id = st.text_input("Telegram chat ID opcional", value="")
    discord_enabled = st.checkbox("Activar Discord", value=False)
    discord_webhook_url = st.text_input("Discord webhook URL", value="", type="password")

    st.divider()
    alert_valid_signals = st.checkbox("Avisar señales válidas", value=True)
    alert_almost_setups = st.checkbox("Avisar setups en formación", value=True)
    alert_bias_change = st.checkbox("Avisar cambio fuerte de sesgo", value=True)
    alert_risk_warning = st.checkbox("Avisar TP/SL demasiado exigente", value=False)

    almost_score_gap = st.slider("Setup en formación: a cuántos puntos del score", 1, 3, 1)
...
SL: {sl:,.0f}
TP: {tp:,.0f}
Distancia SL: {risk_points:,.0f} pts
TP en ATR: {tp_atr_multiple:.2f} ATR
RR: {rr:.2f}
...
try:
    ...
    # Captura automática: guarda solo señales válidas y evita duplicados por vela.
    ...
    if st.button("💾 Guardar señal manual"):
    ...
    with st.expander("📘 Reglas del sistema"):
        ...
        - Auto refresh mantiene el sistema atento durante el día mientras la app esté abierta.
